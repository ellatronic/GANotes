...
let mapsURL = URL(string:kURL)

let task = URLSession.shared.dataTask(with: mapsURL! as URL) { (data,response, error) in
...
//            self.searchResults.text = jsonString
//
//        }
...
let jsonString1 = "{\"aKey\":580681,\"anotherKey\":{\"someNestedKey\":true}}"

if let jsonData = jsonString1.data(using: String.Encoding.utf8) {
    do {
        let json = try JSONSerialization.jsonObject(with: jsonData, options: .allowFragments)
        ...
        if let first = (json as AnyObject)["aKey"] as? Int {
            print(first)
        } else {
            print("Not found")
        }
