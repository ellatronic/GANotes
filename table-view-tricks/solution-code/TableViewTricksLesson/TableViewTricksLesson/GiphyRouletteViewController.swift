...
class GiphyRouletteViewController: UITableViewController {
  ...
  override func viewDidLoad() {
    ...
    //Create the refresh control to enable pull-down-to-refresh
    refreshControl = UIRefreshControl()
    //Set the background color of the refresh area to red
    refreshControl?.backgroundColor = UIColor.red
    //Sets the spinner's color to white
    refreshControl?.tintColor = UIColor.white
    //Tells the refresh controller what function to call when the user pulls down to refresh
    //It uses selectors, which is an Objective-C way of passing a function. Unfortunately this API is dated like that.
  ...
  private func loadNewImage() -> GiphyImage? {
    ...
    do {
      ...
      guard let giphyImage = json["data"] as? NSDictionary
        else {
      ...
      return GiphyImage.from(dictionary: giphyImage)
  ...
  override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
    ...
    guard let cell = tableView.dequeueReusableCell(withIdentifier: "GiphyCell", for: indexPath) as? GiphyCell else {
      ...
      if let image = self.loadImage(atURL: giphy.imageURL) {
        ...
        self.main.addOperation {
          ...
          if (tableView.indexPathsForVisibleRows?.contains(indexPath) ?? false) {
            ...
            //This closure updates the GiphyImageView with the new image
            //It gets run later when we pass it to the `UIView.animate...` method.
            let animations =  {
              cell.giphyImageView.image = image
            }
            
            //This line animates the change of the setting the image using a clean `CrossDissolve` animation
            //It takes 0.5 seconds to complete.
            UIView.transition(with: cell.giphyImageView, duration: 0.5, options: .transitionCrossDissolve, animations: animations, completion: nil)
          }
        }
      }
    }
